...
export class ServicioCalculoDesgaste {
    public calcular(registros: RegistroKilometraje[]): TasaDesgaste {
        ...
        const kmRecorridos = ultimoRegistro.getValorKm() - primerRegistro.getValorKm();
        if (kmRecorridos < 0) {
            throw new Error("El historial de kilometraje presenta inconsistencias cronológicas.");
        }

        const milisegundosPorSemana = 1000 * 60 * 60 * 24 * 7;
        const tiempoSemanas = (ultimoRegistro.getFecha().getTime() - primerRegistro.getFecha().getTime()) / milisegundosPorSemana;

        const kmSemanales = tiempoSemanas > 0.1 ? kmRecorridos / tiempoSemanas : kmRecorridos;
