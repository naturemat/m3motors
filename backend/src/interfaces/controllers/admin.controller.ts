...
export class AdminController {
  ...
  async getKpis(@Req() req: Request) {
    ...
    if (!workshop) {
      return { kpis: { totalVehicles: 0, totalClientsActive: 0, monthlyRevenue: 0, avgMechanicRating: 0, servicesCount: 0 } }
    }

    const clientsActive = await this.prisma.client$.cliente.count({
      where: { idMecanicoActivo: { in: workshop.mecanicos.map((m: any) => m.id) }, status: 'ACTIVATED' },
    })

    const vehicleCount = await this.prisma.client$.vehicle.count({
      where: { idMecanicoActivo: { in: workshop.mecanicos.map((m: any) => m.id) } },
    })

    const monthlyRevenue = await this.prisma.client$.intervention.aggregate({
      _sum: { manoDeObra: true },
      where: { mecanicoId: { in: workshop.mecanicos.map((m: any) => m.id) }, createdAt: { gte: new Date(new Date().getFullYear(), new Date().getMonth(), 1) } },
    })

    const avgMechanicRating = workshop.mecanicos.length
      ? workshop.mecanicos.reduce((sum: number, mech: any) => sum + (mech.rating ?? 0), 0) / workshop.mecanicos.length
      : 0
  ...
  async updateWorkshop(@Req() req: Request, @Body() dto: UpdateWorkshopDTO) {
    const { userId } = (req as any).auth
    const workshop = await this.prisma.client$.workshop.findFirst({ where: { ownerId: userId } })
  ...
  async getMechanics(@Req() req: Request) {
    ...
    const mechanics = await this.prisma.client$.mechanic.findMany({ where: { workshopId: workshop.id } })
    return { mechanics }
  }
  ...
  async deleteMechanic(@Param('id', ParseIntPipe) id: number, @Req() req: Request) {
    const { userId } = (req as any).auth
    const workshop = await this.prisma.client$.workshop.findFirst({ where: { ownerId: userId } })
    if (!workshop) return { error: 'Taller no encontrado' }

    await this.prisma.client$.mechanic.deleteMany({ where: { id, workshopId: workshop.id } })
    return { success: true }
  }
  ...
  async getServices(@Req() req: Request) {
    ...
    const services = await this.prisma.client$.serviceCatalog.findMany({ where: { workshopId: workshop.id } })
    return { services }
  }
  ...
  async activateCustomer(@Param('id', ParseIntPipe) id: number, @Req() req: Request) {
    const { userId } = (req as any).auth
    const workshop = await this.prisma.client$.workshop.findFirst({ where: { ownerId: userId }, include: { mecanicos: true } })
    if (!workshop) return { error: 'Taller no encontrado' }
    ...
    await this.prisma.client$.preRegisteredCustomer.update({
      ...
      data: { status: 'ACTIVATED', fechaActivacion: new Date(), idClienteConverted: cliente.id },
    })
