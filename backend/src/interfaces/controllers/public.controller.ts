...
import { Controller, Get, Post, Param, Body, HttpCode, Inject, Headers, BadRequestException } from '@nestjs/common'
...
export class PublicController {
  ...
  async preRegister(@Param('id') id: string, @Body() body: any, @Headers('x-recaptcha-token') recaptchaToken?: string) {
    ...
    try {
      ...
      if (secret) {
        if (!recaptchaToken) throw new BadRequestException('Missing recaptcha token')
    ...
    try {
      ...
      await this.eventPublisher.publish(ClientePreRegistradoEvent.EVENT_NAME, evento.toPayload())
    } catch (err) {
