...
describe('Cliente Entity', () => {
  ...
  beforeEach(() => {
    cliente = new Cliente(new ClienteId('uuid-1'), 'Juan Perez', '0999999999', 'juan@test.com', 'chat123');
  });
  ...
  it('debe lanzar error al asociar un vehículo que ya existe', () => {
    const placa = new Placa('XYZ-987');
    cliente.asociarNuevoVehiculo(placa);
    expect(() => cliente.asociarNuevoVehiculo(placa)).toThrow('El vehículo ya está asociado a este cliente');
  });

  it('debe actualizar los canales de contacto', () => {
    cliente.actualizarCanalesContacto('0888888888', 'nuevo@test.com', 'chat456');
  ...
  it('debe lanzar error si el email de contacto es inválido', () => {
    expect(() => cliente.actualizarCanalesContacto('0888', 'email-invalido', '123')).toThrow('Formato de email inválido');
  });
});
