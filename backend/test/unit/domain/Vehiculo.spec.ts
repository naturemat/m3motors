...
describe('Vehiculo Aggregate', () => {
  ...
  it('debe lanzar error si faltan datos basicos obligatorios', () => {
    expect(() => new Vehiculo(placa, '', 'Corolla', 2022, 'Gasolina')).toThrow(
      'Marca, modelo y tipo de motor son requeridos',
    );
    expect(() => new Vehiculo(placa, 'Toyota', '', 2022, 'Gasolina')).toThrow(
      'Marca, modelo y tipo de motor son requeridos',
    );
    expect(() => new Vehiculo(placa, 'Toyota', 'Corolla', 2022, '')).toThrow(
      'Marca, modelo y tipo de motor son requeridos',
    );
  });
  ...
  it('debe lanzar error si el año es menor a 1886 o excesivamente futurista', () => {
    ...
    expect(
      () => new Vehiculo(placa, 'Toyota', 'Corolla', 1800, 'Gasolina'),
    ).toThrow('Año de vehículo inválido');
    expect(
      () =>
        new Vehiculo(
          placa,
  ...
  it('debe lanzar error si la fecha del nuevo evento es cronologicamente anterior al ultimo', () => {
    ...
    expect(() =>
      vehiculo.registrarIngresoKilometraje(11000, new Date('2026-05-09')),
    ).toThrow('Inconsistencia cronológica');
  });
  ...
  it('debe lanzar error al intentar modificar una intervencion inexistente', () => {
    ...
    const componente = new ComponenteCritico('Filtro de Aire', 12);

    expect(() =>
      vehiculo.agregarComponenteAIntervencion(idFalso, componente),
    ).toThrow('Intervención no encontrada en el historial');
